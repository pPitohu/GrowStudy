...
function drawTasks() {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        tasks.innerHTML += `
            <div class="task"><i class="ri-pencil-line"></i><span class="pattern">${firstNum} ${operation} ${secondNum} = </span>
...
listOfOperations.forEach((el, i) => {
    el.onclick = (e) => {
        ...
        pickedOperation.innerHTML = `Выбранная операция: <span class="text-success">${el.textContent.slice(
            0,
            -3
        )}</span>`;
    };
...
startButton.onclick = () => {
    if (!operation)
        return (pickedOperation.innerHTML = `<span class="text-danger text-capitalize">Укажите операцию.</span>`);
    if (!difficulty)
        return (pickedDifficulty.innerHTML = `<span class="text-danger text-capitalize">Укажите сложность.</span>`);
...
readyButton.onclick = () => {
    ...
    setTimeout(() => {
        ...
        if (mark > 5) {
            document.querySelector('.done.alert-success').style.display =
                'block';
            document.querySelector(
                '.done.alert-success'
            ).innerHTML = `Успешно!<br>Ваша оценка: ${mark}`;
        } else {
            document.querySelector('.done.alert-danger').style.display =
                'block';
            document.querySelector(
                '.done.alert-danger'
            ).innerHTML = `Не расстраивайтесь!<br>Ваша оценка: ${mark}`;
        }
